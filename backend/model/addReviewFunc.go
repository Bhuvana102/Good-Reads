...
func AddReviewFunc(review views.Review) error {
	...
	DB.Model(views.Book{}).Where("Book_ID = ?", review.Book_ID).UpdateColumn("NoOfRatings", gorm.Expr("NoOfRatings + ?", 1))
	DB.Model(views.Book{}).Where("Book_ID = ?", review.Book_ID).UpdateColumn("NoOfReviews", gorm.Expr("NoOfReviews + ?", 1))
	return nil
}
